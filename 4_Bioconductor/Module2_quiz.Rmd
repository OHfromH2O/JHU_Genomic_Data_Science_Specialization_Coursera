...
# download and load the hg19
library(BiocManager)
# install packages through => BiocManager::install("BSgenome.Hsapiens.UCSC.hg19")
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
# query the record
ah_H3K27me3_fc <- query(ah, c("H3K27me3", "fc.signal", "E003"))
ah_H3K27me3_fc
ah_H3K27me3_fc_record <- ah_H3K27me3_fc[["AH32033"]]

# subset data on chr22, using import
gr22 <- GRanges(seqnames = "chr22", ranges = IRanges(start = start(Hsapiens$chr22), end = end(Hsapiens$chr22)))
H3K27me3_fc_gr <- import(ah_H3K27me3_fc_record, which = gr22, as = "Rle")
...
### -CpG Islands are dense clusters of CpGs. The classic definition of a CpG Island compares the observed to the expected frequencies of CpG dinucleotides as well as the GC content. Specifically, the observed CpG frequency is just the number of “CG” dinucleotides in a region. The expected CpG frequency is defined as the frequency of C multiplied by the frequency of G divided by the length of the region.
...
library(BiocManager)
# install packages through => BiocManager::install("TxDb.Hsapiens.UCSC.hg19.knownGene")
