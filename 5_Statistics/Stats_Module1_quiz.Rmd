...
### Reproducibility is defined informally as the ability to recompute data analytic results conditional on an observed data set and knowledge of the statistical pipeline used to calculate them. (Peng 2011, Science) 
### Replicability of a study is the chance that a new experiment targeting the same scientific question will produce a consistent result. (Asendorpf 2013 European Journal of Personality)

### Susan asks Joe for his data shared according to the data sharing plan discussed in the lectures. 
### **Q1. Which of the following are reasons the study may be reproducible, but not replicable?**  
...
```{r eval=FALSE}
x = rnorm(10) #The Normal Distribution
plot(x,pch=19,col="dodgerblue")
```
```{r eval=FALSE}
y = rbinom(20,size=1,prob=0.5) #The Binomial Distribution
table(y)
```
...
### **Q3. How do you access the genomic data for this object? How do you access the phenotype table? How do you access the feature data? What is the unique additional information provided by rowRanges(se)?**
```{r}
...
### 4. Suppose that you have measured ChIP-Seq data from 10 healthy individuals and 10 metastatic cancer patients. For each individual you split the sample into two identical sub-samples and perform the ChIP-Seq experiment on each sub-sample. 
### **Q4. How can you measure (a) biological variability, (b) technical variability and (c) phenotype variability.**
...
### Imagine the question of interest in the Bottomly data is to compare strains and in the Bodymap data it is to compare tissues.
...
### Answer for Q6:
Humans are intuitively much worse at comparing angles than comparing position or length.
...
### Answer for Q7:
```{r}
row_sums = rowSums(edata) #each row refers to gene
edata = edata[order(-row_sums),] #order in descending manner
index = 1:500 #top 500 highly expressed genes
...
edata = exprs(bm)
```
### Make an MA-plot of the first sample versus the second sample using the log2 transform (hint: you may have to add 1 first) and the rlog transform from the DESeq2 package. 
### **Q8. How are the two MA-plots different? Which kind of genes appear most different in each plot?**
```{r}
# make a MA-plot
...
```{r}
library(DESeq2)
rlt <- rlog(edata) #rlt refers to rlog transformation
y_rlt = rlt[,1] - rlt[,2]
x_rlt = rld[,1] + rlt[,2]
plot(x_rlt, y_rlt, col = "blue", type = "p")
```
...
Color the samples by which study they came from 
##### (Hint: consider using the function myplclust.R in the package rafalib available from CRAN and looking at the argument lab.col.)
...
par(mar=c(0, 4, 4, 2)) #set bottom line of margin to be zero
plot(hclust1, hang = -1, main="original", labels=FALSE)
```
```{r}
# filtering all genes with rowMeans less than 100
low_genes = rowMeans(edata) < 100
filter_edata = filter(edata, !low_genes)
f_dist1 = dist(t(filter_edata))
f_hclust1 = hclust(f_dist1)
par(mar=c(0, 4, 4, 2))
plot(f_hclust1, hang = -1, main="low expression removed", labels=FALSE)
```
```{r}
# taking the log2 transform of the data without filtering
log_edata = log2(edata + 1)
l_dist1 = dist(t(log_edata))
l_hclust1 = hclust(l_dist1)
par(mar=c(0, 4, 4, 2))
plot(l_hclust1, hang=-1, main="log2 transformation", labels=FALSE)
```
...
### Cluster the samples using k-means clustering after applying the log2 transform (be sure to add 1). 
### Set a seed for reproducible results (use set.seed(1235)). 
...
```{r}
#log2 transformation; add +1 to avoid negative infinity values (-inf)
edata = log2(edata + 1)

# perform k-means clustering as two cluster centers
set.seed(1235)
...
# cluster samples into two groups, by using cutree function
